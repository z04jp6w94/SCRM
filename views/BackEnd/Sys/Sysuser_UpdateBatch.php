...
for ($i = 1; $i <= $currentPageRecord; $i++) {
    if (!empty($_REQUEST["rowNum" . $i])) {
        $user_id = $_REQUEST["rowNum" . $i];
        $user_account = $_REQUEST["user_account" . $i];
        $user_name = $_REQUEST["user_name" . $i];
        $user_email = $_REQUEST["user_email" . $i];
        $user_phone = $_REQUEST["user_phone" . $i];
        $sql = "UPDATE sysuser SET user_account = ?, user_name = ?, user_email = ?, user_phone = ? WHERE user_id = ?";
        $mysqli->updatePreSTMT($sql, "sssss", array($user_account, $user_name, $user_email, $user_phone, $user_id));
    }
}
//回原本畫面
